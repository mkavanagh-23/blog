+++
title = 'Hugo, or How I Learned to Automate my Blog Deployments'
description = 'A brief walkthrough into configuring and automating a simple blogging platform using Hugo and NGINX'
...
There is no better candidate for a first post than going over the process of setting up this very blog: a simple, statically generated site using [Hugo](https://gohugo.io/) as a framework, managed and built automatically using webhooks to a remote VPS, all proxied via [NGINX](https://nginx.org/). You can check out all the code on my [Github](https://github.com/mkavanagh-23/blog), it's quite a bit simpler than I'd envisioned it would be.
...
3. Enable the site by linking it to the sites enabled-directory, then test and reload the server:
...
1. Create the file `/usr/local/bin/deploy-hugo.sh` and add the following contents:
    ...
    # Build the site
    HUGO_LOG=~/hugo-site/hugo.log hugo --cleanDestinationDir
    ```
...
2. Build the hook for requests from github. Create the file `~/hooks/hugo.json` and add the following contents:
...
3. Now start the web server temporarily to test that it is receiving HTTP requests:
...
Useful Links:
[Markdown Cheat Sheet](https://www.markdownguide.org/cheat-sheet/) | [GitHub Repo](https://github.com/mkavanagh-23/blog)
